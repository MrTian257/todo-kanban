...
## 废止说明
2026-09-08 后续决策：为简化首启与部署，取消 db-config.txt 指示文件，改为固定使用程序运行目录下的 `todo-kanban.db`。`MCP_TODO_DB_CONFIG` / `--db-config` 语义由「含 db-config.txt 的目录」调整为「含 todo-kanban.db 的目录」。详见 ADR-012。

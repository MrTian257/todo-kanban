...
pub const MIGRATION_STEPS: [(&str, &str); 11] = [
    ...
    ("v10→v11", "待办自定义字段值：todos.custom_fields"),
    (
        "v11→v12",
        "资料库手工排序：resources.sort_order + 存量按插入顺序倒序回填（新在前）",
    ),
];
...
- 自定义字段与自动脚本（v11，ADR-014）：可全局（或按项目）定义新属性、配置值来源（手动 / 内置属性 / 自动脚本），

...
| [02-development/backend-contract.md](02-development/backend-contract.md) | 前后端 | Tauri 命令硬契约（当前 39 个，见 `src-tauri/src/commands.rs`）+ **MCP 11 tools / 5 resources** 契约 |
...
- **扩展**：`src-tauri/mcp-server`（stdio MCP server）：11 tools + 5 resources（含 v11 `todo-kanban://fields`），复用 `todo-kanban-core::svc`，零 tauri 依赖，`MCP_TODO_READONLY=1` 一键只读

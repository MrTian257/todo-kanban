...
export const TodoRow = React.memo(function TodoRow({ todo, projectName, showProjectName, variant = "list" }: Props) {
  ...
  return (
    <div data-todo-id={todo.id} className={cn("group", variant === "card" ? "tk-task-card" : "tk-task-row")}>
      ...
      <div className="tk-task-body min-w-0 flex-1">
        ...
        {todo.branch && <DropdownMenu open={branchMenu} onOpenChange={setBranchMenu}>
          <DropdownMenuTrigger asChild><button className="tk-task-branch mt-2 flex max-w-full items-center gap-2 rounded text-xs text-muted-foreground hover:text-primary" onClick={() => void openBranchMenu()} title={`任务分支：${todo.branch}`}><GitBranch className="h-3.5 w-3.5 shrink-0"/><span className="truncate">{todo.branch}</span></button></DropdownMenuTrigger>

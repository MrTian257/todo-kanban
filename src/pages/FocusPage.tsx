...
export function FocusPage() {
  ...
  // 「今天创建」的边界与标题一起跟跨天刷新，长期驻留时不会停在昨天
  const today=useToday();
  const [showDone,setShowDone]=useState(false);
  const projectById=useMemo(()=>new Map(projects.filter(p=>!p.archived).map(p=>[p.id,p])),[projects]);
  const focused=useMemo(()=>todos.filter(t=>!t.archived && projectById.has(t.projectId) && (t.status==="doing" || t.createdAt>=dayStartMs(today))).sort((a,b)=>a.createdAt-b.createdAt),[todos,projectById,today]);
  ...
  return <div className="tk-page h-full overflow-y-auto"><div className="w-full">
    <div className="tk-eyebrow flex items-center gap-2"><CalendarDays className="h-3.5 w-3.5"/>{new Intl.DateTimeFormat('zh-CN',{month:'long',day:'numeric',weekday:'long'}).format(new Date(dayStartMs(today)))}</div>

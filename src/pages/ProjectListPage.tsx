...
export function ProjectListPage() {
  ...
  return <div className="tk-page h-full overflow-y-auto"><div className="w-full">
    ...
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-2 2xl:grid-cols-3">{visible.map(p=>{const count=counts.get(p.id) ?? {total:0,doing:0,done:0};return <article key={p.id} className="tk-panel p-6 transition-shadow hover:shadow-md">
      <div className="mb-5 flex items-start gap-3"><span className="rounded-xl bg-primary/8 p-3 text-primary"><FolderKanban className="h-5 w-5"/></span><div className="min-w-0 flex-1 pt-1"><div className="flex items-center gap-2"><Link to={`/project/${p.id}`} onClick={() => setActiveProjectId(p.id)} className="block min-w-0 truncate text-base font-semibold hover:text-primary">{p.name}</Link>{p.createdBy === "ai" && <Badge variant="outline" className="shrink-0 gap-1 px-1.5 text-[10px] font-normal text-primary"><Bot className="h-3 w-3"/>AI 创建</Badge>}</div><div className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground"><GitBranch className="h-3 w-3"/><span className="truncate font-mono">{p.productionBranch||"未设置生产分支"}</span></div></div>
      <DropdownMenu><DropdownMenuTrigger asChild><Button aria-label={`管理项目 ${p.name}`} variant="ghost" size="icon" className="h-8 w-8"><MoreHorizontal className="h-4 w-4"/></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={()=>{setEditing(p);setOpen(true);}}><Pencil className="h-4 w-4"/>编辑项目</DropdownMenuItem><DropdownMenuItem onClick={()=>{upsertProject({...p,archived:!p.archived,updatedAt:Date.now()});void flushPersistence().then(()=>toast.success(p.archived?"项目已恢复":"项目已归档")).catch(error=>toast.error(String(error)));}}>{p.archived?<ArchiveRestore className="h-4 w-4"/>:<Archive className="h-4 w-4"/>}{p.archived?'恢复项目':'归档项目'}</DropdownMenuItem><DropdownMenuSeparator/><DropdownMenuItem className="text-destructive" onClick={()=>setDeleteTarget(p)}><Trash2 className="h-4 w-4"/>删除项目</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>
      ...
      <div className="mt-5 flex items-center justify-between border-t pt-4 text-xs"><span className="text-muted-foreground">{p.swimlanes?.length??0} 个泳道</span><Link to={`/project/${p.id}`} onClick={() => setActiveProjectId(p.id)} className="flex items-center gap-1.5 font-medium text-primary">进入看板<ArrowRight className="h-3.5 w-3.5"/></Link></div>
    </article>;})}</div>
